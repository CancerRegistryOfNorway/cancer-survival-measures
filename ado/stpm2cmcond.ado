...
program define stpm2cmcond
	...
	syntax using/ , [AT(string) MERGEBY(string) DIAGAge(int 50) DIAGYear(int 1990) SEX(int 1) ATTAge(name) ///
						...
						MERGEGEN(string) ADDLHR(real 0) survprob(name) tcond(real 0)]
...
/* Other merge variables - Check this is done using mergegen */									
	...
	*GENERATE OTHER SPLINES*/
	...
	quietly su s_all, meanonly
	replace s_all=s_all/`r(max)'
	qui predict double lambda , hazard 
	quietly su `St_star', meanonly
	replace `St_star'=`St_star'/`r(max)'	
	qui gen double  St_starXft = s_all*lambda
	qui gen double RtXS_starXlambda_star = s_all*`rate'
